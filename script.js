...
// var apiUrl = "https://v2.jokeapi.dev/joke/Programming,Miscellaneous,Dark,Pun?type=single";
var apiUrl = "https://v2.jokeapi.dev/joke/Any?type=twopart";
...
async function getJoke(){
    ...
    try{
        ...
        return {setup:jokeSetup,delivery:jokeDelivery};
    }catch(error){
        console.error(error);
    }
}
...
jokeButton.addEventListener('click',async function(){
    ...
    if('speechSynthesis' in window){
        ...
        const myText1 = new SpeechSynthesisUtterance();
        myText1.text = myjoke.setup;
        myText1.voice = speechSynthesis.getVoices()[0];
        myText1.rate = 1;
        myText1.pitch = 1;
        myText1.volume = 1;


        const myText2 = new SpeechSynthesisUtterance();
        myText2.voice = speechSynthesis.getVoices()[0];
        myText2.text = myjoke.delivery;
        myText2.rate = 1;
        myText2.pitch = 1;
        myText2.volume = 1;
        
        speechSynthesis.speak(myText1);
        setTimeout(() => {
            speechSynthesis.speak(myText2);
        }, 3000);
        
        myText2.onend = function() {
